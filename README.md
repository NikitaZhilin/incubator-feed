...
- Замес смеси: расчет доступных замесов по складу, выбор пшеницы или зерносмеси, режим `Сделать сейчас` с чек-листом или `Записать как уже скормленный` для заднего числа.
...
- Учет яиц: обычный сбор, сбор за несколько дней с ручным или автоматическим периодом, история, прогноз по последним 7/30 дням, исключение несушек, которые временно не несутся, погодная поправка через Open-Meteo.
- Птицевод-практик: единый советник по данным хозяйства с планом на сегодня, подсказками по кормам и замесу, анализом просадки яиц, задачами по инкубации и безопасным сценарием при проблемах с птицей.
...
- Встроенные инструкции: кнопки `FAQ` читают тексты из отдельных файлов `app/content/help/*.md`.
...
- `🥚 Яйца`
  - обычный сбор, сбор за несколько дней, расчеты, история, временно не несущиеся куры, город и погода
- `🐔 Птицевод`
  - план на сегодня, корма и замес, мало яиц, инкубация сегодня, проблема с птицей
- `⚙️ Настройки`
...
- `DOCS_URL` - ссылка на документацию для `Настройки -> О боте`.
...
Основные команды: `/start`, `/menu`, `/share`, `/help`, `/version`, `/advisor`, `/new`, `/batches`, `/today`, `/history`, `/stats`, `/profiles`, `/calendar`, `/care`, `/feed`, `/settings`, `/remind 09:00`, `/remind off`, `/timezone Europe/Moscow`, `/farm Название`, `/units metric`, `/disclaimer`, `/cancel`.
...
настраивает компактный Telegram menu button `АПП` при старте. Без `WEB_PUBLIC_URL`
...
- [Project state audit 2026-07-11](docs/PROJECT_STATE_AUDIT_2026-07-11.md)
- [Bot help files](docs/BOT_HELP_FILES.md)
...
- [Poultry advisor agent](docs/POULTRY_ADVISOR_AGENT.md)
- [Poultry advisor implementation plan](docs/POULTRY_ADVISOR_IMPLEMENTATION_PLAN.md)
