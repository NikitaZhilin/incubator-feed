...
Погода для раздела `Яйца` загружается из Open-Meteo по названию города. Отдельный API key не нужен. Открытие раздела использует сохраненные погодные данные; сетевое обновление запускается только вручную.
