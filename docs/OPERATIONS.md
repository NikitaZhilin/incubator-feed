...
10. Для обычного деплоя проверить, что release notice не отправлялся; для среднего/важного релиза отправить retry вручную: `python -B scripts/notify_release.py --version 0.1.42-beta --notes "Кратко об изменениях"`.
